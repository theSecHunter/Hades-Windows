...
| 系统信息     | 系统基础信息(软/硬件信息等)                          |
| 系统日志     | 应用程序日志、安全日志、系统日志数据                 |
| 系统用户     | 系统用户                                             |
| 系统软件服务 | 已安装软件/服务                                      |
| 文件         | 列举指定目录下文件，可与ntfs数据对比.                |
| Rootkit_PE   | Rootkit检测需要调用接口解析相关的PE文件，提取对比源. |
...
| --------------------------------------------------------- | -------------- |
| Rootkit优化/完善                                          | 高             |
| 兼容Win7/Win10 x64大部分版本，非长期稳定驱动测试          | 高             |
...
| 文件备份：引入Minfilter，如Cmd - Powershell - Vbs下载文件，Minifilter自动备份至指定目录 | 高     |
| 优化通信：Win下目前使用Grpc，但是有诸多不变。引入IOCP模块，后续如果server使用Epoll，将联调对接。 | 中     |
