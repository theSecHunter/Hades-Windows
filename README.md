...
- v2.0 Hades_Win兼容Linux数据上报，添加Go_Server完整数据格式解析处理，采集数据目前通过Grpc上报至Go_server进行解析即可，但生产正数据还有很多未清洗。支持Server主动下发采集任务，Rootkit数据采集平台有待完善。支持Server管理Client监控/开启，Go_Server还未引入主分支详细见: https://github.com/theSecHunter/Hades-Linux
