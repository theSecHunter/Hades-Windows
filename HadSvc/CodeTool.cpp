...
string CodeTool::EncodeBase64(const unsigned char * str, int bytes)
{
	...
	if (bytes > 0)
	{
		_encode_result += _base64_table[current[0] >> 2];
		if (bytes % 3 == 1) {
			_encode_result += _base64_table[static_cast<size_t>((static_cast<unsigned int>(current[0]) & 0x03u) << 4u)];
			_encode_result += "==";
		}
		else if (bytes % 3 == 2) {
			_encode_result += _base64_table[static_cast<size_t>(((static_cast<unsigned int>(current[0]) & 0x03u) << 4u) + (static_cast<unsigned int>(current[1]) >> 4u))];
			_encode_result += _base64_table[static_cast<size_t>((static_cast<unsigned int>(current[1]) & 0x0fu) << 2u)];
			_encode_result += "=";
		}
	}
...
void CodeTool::DecodeBase64(const char *str, int bytes, char*& dest, int& len)
{
	...
	if (_decode_result.length() > static_cast<size_t>((std::numeric_limits<int>::max)() - 1))
		return;

	len = static_cast<int>(_decode_result.length());
	dest = new char[static_cast<size_t>(len) + 1]();
	if (len > 0)
		memcpy(dest, _decode_result.data(), static_cast<size_t>(len));
}
...
void CodeTool::WriteRegEditToPluGinVersion(const std::wstring& strPluGinName, const std::wstring& strVersion)
{
	try
	{
		...
		lRet = RegSetValueEx(
			...
			static_cast<DWORD>(sizeof(wchar_t) * (strVersion.size() + 1))
			);
		RegCloseKey(hKey);
	}
	catch (...)
