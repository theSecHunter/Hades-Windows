...
DWORD WINAPI ReadThread(LPVOID param) {
	...
	std::unique_ptr<char[]> buffer(new (std::nothrow) char[PIPE_BUFSIZE]);
	if (!buffer) {
	...
	while (!client->exit) {
		...
		BOOL success = ReadFile(client->handle, buffer.get(), PIPE_BUFSIZE, &dwRead, &ovlp);
		if (!success) {
			...
			if (!GetOverlappedResult(client->handle, &ovlp, &dwRead, FALSE)) {
				break;
			}
		}
		...
		std::string recvbuf(buffer.get(), dwRead);
...
DWORD WINAPI ListenThread(LPVOID param) {
	...
	while (!server->exit) {
		...
		BOOL connected = ConnectNamedPipe(handle, &ovlp);
		if (!connected) {
			DWORD error = GetLastError();
			if (error == ERROR_PIPE_CONNECTED) {
				...
				while (!server->exit) {
					...
					if (wait == WAIT_OBJECT_0) {
						...
						connected = GetOverlappedResult(handle, &ovlp, &dwTransferred, FALSE);
						break;
					}
...
void PipeManage::ServerStop()
{
	...
	while (!IsListEmpty(&m_serverHead.link)) {
		...
		while (!IsListEmpty(&server->client.link)) {
			...
			// 关闭客户端管道
			if (client->handle && client->handle != INVALID_HANDLE_VALUE) {
...
bool PipeManage::ServerSend(const std::string& name, const std::string& str)
{
	...
	while (_EX_Blink != &server->client.link) {
		...
		DWORD dwWrite = 0;
		BOOL success = WriteFile(client->handle, str.c_str(), (DWORD)str.size(), &dwWrite, NULL);
		if (success && dwWrite == str.size()) {
