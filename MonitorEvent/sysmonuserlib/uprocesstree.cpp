	...
	if (EnumProcessModules(hProcess, hMods, sizeof(hMods), &cbNeeded))
	{
		for (i = 0; i < (cbNeeded / sizeof(HMODULE)); i++)
		{
				...
				sizeof(szModName) / sizeof(TCHAR)))
			{
				...
				_tprintf(TEXT("\t%s (%p)\n"), szModName, reinterpret_cast<void*>(hMods[i]));
...
void GetQueryViryualMemoryStatue(HANDLE hProccess)
{
	...
	while (1) {
		...
		bRet = VirtualQueryEx(hProccess,
			reinterpret_cast<LPCVOID>(dwAddress),
			&mbi,
		...
		switch (statue.state) {
		case e_stat_free:
			str.Format(L"free:          0x%llX", static_cast<unsigned long long>(statue.address));
			break;
		case e_stat_reserve:
			str.Format(L"reserve:   0x%llX", static_cast<unsigned long long>(statue.address));
			break;
		case e_stat_commit:
			str.Format(L"commit:   0x%llX", static_cast<unsigned long long>(statue.address));
			break;
		}
		...
		if (statue.state == e_stat_commit) {
			...
			ULONG_PTR dwBlockAddress = dwAddress;
			while (1) {

				bRet = VirtualQueryEx(hProccess,
					reinterpret_cast<LPCVOID>(dwBlockAddress),
					&mbi,
					sizeof(MEMORY_BASIC_INFORMATION));
				if (!bRet) {
					break;
				...
				if (mbi.AllocationBase != dwAllocationBase)
					break;
				// 添加RVA
				str.Format(L"0x%llX", static_cast<unsigned long long>(dwBlockAddress));
				...
				// 输出内存块的大小.
				// printf(" 大小:0x%X\n", mbi.RegionSize);
				str.Format(L"0x%llX", static_cast<unsigned long long>(mbi.RegionSize));
