...
namespace DuiLib
{
	...
	void CHorizontalLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
      ...
      cxFixedRemaining = cxFixedRemaining - (rcPadding.left + rcPadding.right);
		...
		bool isAnyItemFloot = false;
		//for (int i = 0; i < m_items.GetSize(); i++)
		//{
		//	if (static_cast<CControlUI*>(m_items[i])->IsFloat())
		//	{
		//		isAnyItemFloot = true;
		//		break;
		//	}
		//}
		//if (!isAnyItemFloot && m_items.GetSize() > 0)
		//{
		//	LONG left = static_cast<CControlUI*>(m_items[0])->GetPos().left;
		//	LONG right = static_cast<CControlUI*>(m_items[m_items.GetSize() - 1])->GetPos().left;
		//	auto v1 = static_cast<CControlUI*>(m_items[0])->GetName();
		//	auto v2 = static_cast<CControlUI*>(m_items[m_items.GetSize() - 1])->GetName();
		//	LONG dValue = (this->GetWidth() - (right - left)) / 2;
		//	if (dValue > 0)
		//	{
