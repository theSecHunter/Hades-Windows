...
namespace DuiLib
{
	...
	void CVerticalLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
      ...
      cyFixedRemaining = cyFixedRemaining - (rcPadding.top + rcPadding.bottom);
		...
		//{
		//	LONG top = static_cast<CControlUI*>(m_items[0])->GetPos().left;
		//	LONG bottom = static_cast<CControlUI*>(m_items[m_items.GetSize() - 1])->GetPos().left;
		//	LONG dValue = (this->GetHeight() - (bottom - top)) / 2;
		//	if (dValue > 0)
		//	{
