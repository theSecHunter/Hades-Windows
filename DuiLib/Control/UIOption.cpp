...
namespace DuiLib
{
	...
	{
		::ZeroMemory(&m_rcSelectedBorderSize, sizeof(RECT));
	}
	...
	void COptionUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("selectedbordersize")) == 0) {
			CDuiString nValue = pstrValue;
			if (nValue.Find(',') < 0) {
				SetSelectedBorderSize(_ttoi(pstrValue));
			}
			else {
				RECT rcBorder = { 0 };
				LPTSTR pstr = NULL;
				...
				SetSelectedBorderSize(rcBorder);
			}
		}
...
Label_ForeImage:
	...
	void COptionUI::SetSelectedBorderColor(DWORD dwColor)
	{
		if (m_dwSelectedBorderColor == dwColor) return;
		m_dwSelectedBorderColor = dwColor;
		Invalidate();
	}
	DWORD COptionUI::GetSelectedBorderColor() const
	{
		return m_dwSelectedBorderColor;
	}
	RECT COptionUI::GetSelectedBorderSize() const
	{
		return m_rcSelectedBorderSize;
	}
	void COptionUI::SetSelectedBorderSize(RECT rc)
	{
		m_rcSelectedBorderSize = rc;
		Invalidate();
	}
	void COptionUI::SetSelectedBorderSize(int iSize)
	{
		m_rcSelectedBorderSize.left = m_rcSelectedBorderSize.top = m_rcSelectedBorderSize.right = m_rcSelectedBorderSize.bottom = iSize;
		Invalidate();
	}
	void COptionUI::PaintBorder(HDC hDC)
	{
		if ((m_uButtonState & UISTATE_SELECTED) != 0) {
			if (m_dwSelectedBorderColor != 0) {
				if (m_cxyBorderRound.cx > 0 || m_cxyBorderRound.cy > 0) {
					CRenderEngine::DrawRoundRect(hDC, m_rcItem, m_rcSelectedBorderSize.left, m_cxyBorderRound.cx, m_cxyBorderRound.cy, GetAdjustColor(m_dwSelectedBorderColor), m_nBorderStyle);
				}
				else {
					if (m_rcSelectedBorderSize.right == m_rcSelectedBorderSize.left && m_rcSelectedBorderSize.top == m_rcSelectedBorderSize.left && m_rcSelectedBorderSize.bottom == m_rcSelectedBorderSize.left) {
						CRenderEngine::DrawRect(hDC, m_rcItem, m_rcSelectedBorderSize.left, GetAdjustColor(m_dwSelectedBorderColor), m_nBorderStyle);
					}
					else {
						RECT rcBorder;
						if (m_rcSelectedBorderSize.left > 0){
							rcBorder = m_rcItem;
							rcBorder.right = rcBorder.left;
							CRenderEngine::DrawLine(hDC, rcBorder, m_rcSelectedBorderSize.left, GetAdjustColor(m_dwSelectedBorderColor), m_nBorderStyle);
						}
						if (m_rcSelectedBorderSize.top > 0) {
							rcBorder = m_rcItem;
							rcBorder.bottom = rcBorder.top;
							CRenderEngine::DrawLine(hDC, rcBorder, m_rcSelectedBorderSize.top, GetAdjustColor(m_dwSelectedBorderColor), m_nBorderStyle);
						}
						if (m_rcSelectedBorderSize.right > 0) {
							rcBorder = m_rcItem;
							rcBorder.right -= 1;
							rcBorder.left = rcBorder.right;
							CRenderEngine::DrawLine(hDC, rcBorder, m_rcSelectedBorderSize.right, GetAdjustColor(m_dwSelectedBorderColor), m_nBorderStyle);
						}
						if (m_rcSelectedBorderSize.bottom > 0) {
							rcBorder = m_rcItem;
							rcBorder.bottom -= 1;
							rcBorder.top = rcBorder.bottom;
							CRenderEngine::DrawLine(hDC, rcBorder, m_rcSelectedBorderSize.bottom, GetAdjustColor(m_dwSelectedBorderColor), m_nBorderStyle);
						}
					}
				}
			}
