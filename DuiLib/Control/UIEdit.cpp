...
namespace DuiLib
{
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditbkColor(0xFFFFFFFF), m_strSpaceholder(_T("")), m_dwSpaceholderColor(0), m_dwDisSpaceholderColor(0), m_iWindowStyls(0)
	{
	...
	void CEditUI::SetSpaceholder(LPCTSTR pStrSpaceholder)
	{
		m_strSpaceholder = pStrSpaceholder;
	}

	void CEditUI::SetSpaceholderColor(DWORD dwSpaceholderColor)
	{
		m_dwSpaceholderColor = dwSpaceholderColor;
	}

	void CEditUI::SetDisabledSpaceholderColor(DWORD dwDisSpaceholderColor)
	{
		m_dwDisSpaceholderColor = dwDisSpaceholderColor;
	}
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("spaceholder")) == 0) SetSpaceholder(pstrValue);
		else if (_tcscmp(pstrName, _T("spaceholdercolor")) == 0){
			if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
			SetSpaceholderColor(clrColor);
		}
		else if (_tcscmp(pstrName, _T("disabledspaceholdercolor")) == 0){
			if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
			SetDisabledSpaceholderColor(clrColor);
		}
	...
	{
		DWORD curTextColor, curDisabledTextColor;
		CDuiString sText;

		if (m_sText.IsEmpty())
		{
			if (m_strSpaceholder.IsEmpty()) return;
			else sText = m_strSpaceholder;

			if (m_dwSpaceholderColor == 0) curTextColor = m_pManager->GetDefaultDisabledColor();
			else curTextColor = m_dwSpaceholderColor;
			if (m_dwDisSpaceholderColor == 0) curDisabledTextColor = m_pManager->GetDefaultDisabledColor();
			else curDisabledTextColor = m_dwDisSpaceholderColor;
		}
		else
		{
			sText = m_sText;
			...
			if (m_dwTextColor == 0) curTextColor = m_pManager->GetDefaultFontColor();
			else curTextColor = m_dwTextColor;
			if (m_dwDisabledTextColor == 0) curDisabledTextColor = m_pManager->GetDefaultDisabledColor();
			else curDisabledTextColor = m_dwDisabledTextColor;
		}
		...
		if( IsEnabled() ) {
			CRenderEngine::DrawText(hDC, m_pManager, rc, sText, curTextColor, \
				m_iFont, DT_SINGLELINE | m_uTextStyle);
		}
		else {
			CRenderEngine::DrawText(hDC, m_pManager, rc, sText, curDisabledTextColor, \
				m_iFont, DT_SINGLELINE | m_uTextStyle);
