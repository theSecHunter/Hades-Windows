...
namespace DuiLib
{
	...
	void CLabelUI::SetAlign(LPCTSTR pstrText)
	{
		if (_tcsstr(pstrText, _T("left")) != NULL) {
			m_uTextStyle &= ~(DT_CENTER | DT_RIGHT);
			m_uTextStyle |= DT_LEFT;
		}
		else if (_tcsstr(pstrText, _T("center")) != NULL) {
			m_uTextStyle &= ~(DT_LEFT | DT_RIGHT);
			m_uTextStyle |= DT_CENTER;
		}
		else if (_tcsstr(pstrText, _T("right")) != NULL) {
			m_uTextStyle &= ~(DT_LEFT | DT_CENTER);
			m_uTextStyle |= DT_RIGHT;
		}
	}

	void CLabelUI::SetVAlign(LPCTSTR pstrText)
	{
		if (_tcsstr(pstrText, _T("top")) != NULL) {
			m_uTextStyle &= ~(DT_BOTTOM | DT_VCENTER);
			m_uTextStyle |= DT_TOP;
		}
		else if (_tcsstr(pstrText, _T("vcenter")) != NULL) {
			m_uTextStyle &= ~(DT_TOP | DT_BOTTOM);
			m_uTextStyle |= DT_VCENTER;
		}
		else if (_tcsstr(pstrText, _T("bottom")) != NULL) {
	...
	{
		if (_tcscmp(pstrName, _T("align")) == 0) SetAlign(pstrValue);
		else if (_tcscmp(pstrName, _T("valign")) == 0) SetVAlign(pstrValue);
	...
	void CLabelUI::PaintText(HDC hDC)
	{
		...
		int iadd = 0;
		if (m_uTextStyle != m_uTextStyle | DT_SINGLELINE) {
